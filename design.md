...
- [ ] 접기/펼치기 **양방향 모두** 제스처 도중에 시작하는지 확인. 한쪽만 제스처가 끝난
      뒤에 발동하면 그 방향만 뚝 끊겨 보입니다. (§5-3)
...
그래서 펼치기는 **위로 스크롤을 시작하면 바로** 발동시킵니다 (누적 상승 48px).
값이 아니라 방향에 반응하게 하는 것이 핵심입니다.

```js
dy<0 ? acc+=dy : acc=0;                       // 위로 간 거리 누적, 아래로 가면 리셋
let next = sc ? !(y<24 || acc<=-48)           // 맨 위이거나, 48px 이상 위로 끌었으면 펼침
              : (k>=300 && y>64);             // 접기는 기존 그대로
```
