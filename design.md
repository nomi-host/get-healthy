...
**단축어로 직행하는 경로(`BKQ0`)** — 공유시트조차 거치지 않으려면 iOS에서 길이 하나뿐입니다.
암호화한 내용을 **클립보드에 싣고** `shortcuts://run-shortcut?name=…&input=clipboard`로 넘깁니다.
주의할 점 셋:

- **클립보드 쓰기는 사용자 제스처를 잃으면 거부됩니다.** `await` 뒤에 `writeText`를 부르면
  Safari가 막습니다. `new ClipboardItem({"text/plain": 약속})`처럼 **Promise를 그대로 넘기는**
  형태로 써야 제스처가 유지됩니다.
- **URL에 데이터를 실으면 안 됩니다.** 백업이 160KB대라 `&text=`로 넘기면 조용히 실패합니다.
- **단축어가 없거나 이름이 다르면 아무 일도 안 일어납니다.** `visibilitychange`로 앱이
  백그라운드로 갔는지 1.5초 지켜보고, 안 갔으면 공유시트(`BKO0`)로 되돌아옵니다.
