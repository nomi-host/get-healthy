...
> (React가 무효 값을 그냥 버림 → UA 기본 테두리가 나옴). §7-7 참고.
...
### 7-6. ⭐ minify된 블록을 통째로 교체할 땐 괄호 수지를 맞출 것
...
### 7-7. ⭐ 전역 토큰에 짧은 이름을 쓰면 minify된 지역변수에 가려진다
...
### 7-10. ⭐ GitHub Pages의 ETag는 "내용이 바뀌었다"는 뜻이 아니다
...
### 7-11. ⭐ `String.fromCharCode.apply(null, 큰배열)`은 RangeError로 죽는다
