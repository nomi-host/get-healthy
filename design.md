...
- [ ] 보더는 **실선(solid) 1px**. 굵기로 위계 만들지 말 것. 색은 `TK0` 램프에서. (§1, §3)
- [ ] 선택 상태는 굵은 테두리가 아니라 **톤 채움 + 옅은 동색 테두리**. (§3)
- [ ] 텍스트 색은 `TK0.t1/t2/t3` 3단 중에서. 새 회색 만들지 말 것. (§1)
...
- [ ] 헤더 트리거를 건드렸다면 **페이지 중간에서 스크롤해도 상태가 안 바뀌는지**, 그리고
      **입력 필드에 포커스가 있는 동안 안 움직이는지** 확인. (§5-3)

### 마무리 (생략 금지)
- [ ] `<script>` 태그 **3개 전부** `node --check`. (CLAUDE.md 함정 1)
- [ ] Playwright로 실제 렌더 스크린샷 확인 + `pageerror`/`console.error` 0건. (CLAUDE.md 함정 1)
...
중립 회색을 그때그때 고르지 말고, **navy에서 파생된 램프 하나**만 쓰세요. 선이 팔레트
바깥의 남의 회색이 아니라 브랜드 색의 옅은 단계로 읽혀야 화면이 정돈돼 보입니다.

```js
var TK0={
  hair  :"rgba(37,58,130,0.07)",   // 카드·행 컨테이너 외곽 (거의 안 보임)
  line  :"rgba(37,58,130,0.11)",   // 평상시 컨트롤 외곽
  lineIn:"rgba(37,58,130,0.15)",   // 입력 요소 외곽 (편집 가능해 보여야 함)
  div   :"rgba(37,58,130,0.06)",   // 구분선
  sel   :"38",                     // 선택 상태 보더 알파 (accent+TK0.sel)
  t1:"#253A82", t2:"#6B7A9E", t3:"#9AA6C0",   // 텍스트 3단
  sh1:"...", sh2:"..."             // 그림자 (§3)
};
```
...
| 토큰 | 용도 |
|---|---|
| `TK0.hair` | 카드·행 컨테이너 외곽 |
| `TK0.line` | 버튼·칩 평상시 외곽 |
| `TK0.lineIn` | input / textarea / select 외곽 |
| `TK0.div` | 구분선, 목록 내부 줄 |
| `TK0.t1` | 본문·제목 (navy) |
| `TK0.t2` | 보조 설명 |
| `TK0.t3` | 메타·비활성 라벨·화살표 |

> ⚠️ **`K`라는 이름은 쓰지 마세요.** 번들 안에 지역변수 `K`가 15곳 넘게 있어서 섀도잉되고,
> `border:\`1px solid ${K.line}\`` 가 조용히 `undefined`가 되어 **보더가 통째로 사라집니다**
> (React가 무효 값을 그냥 버림 → UA 기본 테두리가 나옴). CLAUDE.md 함정 3 참고.
...
2단 그림자를 씁니다. 가까운 층(1~2px)이 형태를 잡고, 먼 층(16~40px)이 공간감을 줍니다.
한 겹짜리 그림자는 번지기만 하고 떠 보이지 않습니다.

```js
TK0.sh1 = "0 1px 2px rgba(37,58,130,0.05), 0 6px 18px rgba(37,58,130,0.06)"   // 섹션 카드
TK0.sh2 = "0 2px 8px rgba(37,58,130,0.10), 0 16px 40px rgba(37,58,130,0.18)"  // 모달
0 4px 16px rgba(0,0,0,.18)                                                    // 업데이트 배너 (예외)
```

### 보더

- **항상 실선.** `dashed`는 현재 코드에 0곳 — 다시 도입하지 마세요.
- **두께는 전부 `1px`.** 예전엔 `1.5px`가 기본이었는데 화면 전체에서 선이 먼저 눈에
  들어와 촌스러웠습니다. 굵기로 위계를 만들지 말고 **투명도로** 만드세요.
- 색은 §1 `TK0` 램프에서 고를 것. 중립 회색(`#d1d5db`, `#e2e8f0`, `#eef0f3`)을 새로
  쓰지 마세요.

**선택 상태는 선이 아니라 면으로.** 선택된 컨트롤을 굵은 테두리로 표시하지 말고,
톤 채움 + 옅은 같은 색 테두리로 표현합니다.

```js
// ✗ 선택: border:`1.5px solid ${c.navy}`
// ✓ 선택: border:`1px solid ${c.navy}${TK0.sel}`, background:`${c.navy}16`
//   평상시: border:`1px solid ${TK0.line}`,       background:"#fff"
```
...
| 약 추가 폼 | **항상 닫힘** (`＋ 약 추가` 버튼) | 목록을 가리지 않게 |
| 행 상세 | **항상 닫힘** | 목록 훑기를 방해하지 않게 |

**선택된 항목은 접힌 그룹 밖으로 꺼내세요.** 필요시 약을 "복용"으로 체크하면 그 약은
그룹 박스 밖(매일 약들 아래)으로 나오고, 그룹에는 아직 안 먹은 것만 남습니다
(`prnOn`/`prnOff`). 접혀 있는 그룹 안에 오늘 기록한 내용이 숨으면 안 됩니다.

**보여줄 상세가 없으면 토글 자체를 렌더하지 마세요.** 빈 상세를 여는 화살표는 노이즈입니다.
복약 행 기준: `hd = items.length > 1 || !!effect || items.some(it => it.s)` 일 때만
"눌러서 상세" 줄과 화살표를 렌더하고, 아니면 부제 줄 전체를 생략하고 `cursor`도 `default`로
둡니다. **성분이 하나뿐이어도 규격(`s`, 예 "500mg")이 있으면 상세를 열 수 있어야 합니다** —
아니면 그 정보가 화면 어디에도 안 나옵니다.
...
접기/펼치기는 **화면 위쪽 좁은 구간에서만** 상태가 바뀝니다.

```js
접기:  y > 120   (그리고 스크롤 가능 높이 k >= 300)
펼치기: y < 8     ← 사실상 "맨 위에 닿았을 때만"
```

**펼치기는 반드시 맨 위(y≈0)에서만.** 중간에서 펼치면 뷰포트 위쪽 문서가 138px 늘어나고,
브라우저가 스크롤 앵커링으로 `scrollY`를 그만큼 보정합니다. 그 값이 접기 임계값을 넘겨
바로 다시 접히고 → 다시 펼쳐지는 **무한 왕복**이 생깁니다. head의 `overflow-anchor:none`은
Chrome/Firefox 전용이라 iOS Safari에서는 막아주지 못합니다.

`y≈0`에서는 위쪽에 보정할 여백 자체가 없어서 루프가 시작되지 않습니다.

**페이지 중간에서는 절대 발동하지 않아야 합니다.** 한때 "위로 48px 끌면 펼침" 같은
방향 기반 트리거를 썼는데, 스크롤에 너무 민감해서 폼을 채우는 동안 헤더가 멋대로 펼쳐지고
그때마다 콘텐츠가 138px 밀려 **입력 자체가 불가능**했습니다. 방향 기반은 쓰지 마세요.

**입력 중에는 아예 얼립니다.** 소프트 키보드가 뜨면 브라우저가 포커스된 필드를 보이게
하려고 페이지를 스크롤하는데, 그게 헤더를 토글하면 레이아웃이 밀리고 → 브라우저가 다시
스크롤하는 악순환이 생깁니다.
...
#### (참고) 펼치기가 "제스처 뒤"에 오면 급작스럽다

커브가 같아도 **접기는 자연스러운데 펼치기만 급작스럽게** 느껴질 수 있습니다. 원인은
애니메이션이 아니라 **언제 발동하느냐**입니다.

- 접기는 `y > 64`에서 발동 → 아직 아래로 스크롤하는 중이라, 헤더가 줄어드는 움직임이
  손가락 움직임에 얹혀 자연스럽게 읽힙니다.
- 펼치기를 `y < 24`(맨 위)에서만 발동시키면, 그 시점엔 **이미 스크롤이 끝나 멈춰 있습니다.**
  138px가 아무것도 안 움직이는 화면에서 단독으로 펼쳐져 뚝 끊겨 보입니다.

부드러움과 안정성이 충돌하면 **안정성이 우선**입니다. 펼치기를 일찍 시작하면 부드럽지만,
그만큼 스크롤 중 레이아웃이 밀려 입력·스크롤이 망가집니다. 실제 사용에서 후자가 훨씬
치명적이라 "맨 위에서만 펼침"으로 확정했습니다.

**피드백 루프 주의**: 헤더가 접히면 문서가 138px 짧아져서, 페이지 맨 아래에 있으면
스크롤 위치가 강제로 당겨지고 그게 "위로 스크롤"로 잡혀 다시 펼쳐지는 무한 반복이 생깁니다.
상태를 바꾼 뒤 **애니메이션 길이만큼(460ms) 스크롤 처리를 무시**해서 막습니다.

**락을 걸 땐 "전부 무시"하지 말 것.** 피드백 루프를 막겠다고 락 동안 스크롤 처리를 통째로
`return` 해버리면, 락이 끝난 뒤 스크롤 이벤트가 더 안 오는 경우 **마지막 위치가 평가되지
않은 채로 굳습니다.** 실제로 "아래로 훑고 곧바로 맨 위로 튕겨 올리기"에서 `scrollY=0`인데
헤더가 접힌 채 멈추는 버그가 났습니다.

- 락 동안에는 **방향 누적값만** 억제하고(클램프가 위조하는 게 그것이므로),
  절대 위치 조건(`y<24`)은 그대로 평가할 것.
- 추가로 스크롤이 멎은 뒤 **한 번 더 재평가**하는 트레일링 타이머(500ms)를 둘 것.

> 검증 항목: ① 위로 끄는 도중에 높이가 단계적으로 커지는지 ② 임계값 근처에서 흔들었을 때
> 상태가 왕복하지 않는지 ③ 페이지 맨 아래에서 접힌 채 안정적인지
> ④ **아래로 스크롤 직후 곧바로 맨 위로 튕겨 올렸을 때 펼쳐지는지**.
...
맨 위에 **액션 1개**, 그 아래 **이동 항목 4개**입니다:

**↓ 아이클라우드 백업** (네온 배경 · 눌리면 그 자리에서 실행)
**투약 스케줄 관리 · 복약 설정 · 데이터 관리 · 목표 수정** (하위 화면으로 이동)

백업만 색이 다른 이유는 **성격이 다르기 때문**입니다 — 나머지 넷은 화면을 여는 이동이고
(`›` 꼬리표가 그 뜻입니다), 백업은 누르는 순간 끝나는 동작이라 꼬리표가 없습니다.
높이 46 · radius 12 · 여백은 이동 항목과 같게 두고 **배경만 `c.neon`**으로 갈랐습니다
(§6-2의 "형태는 같게, 색으로 역할" 관례).
...
## 7. 알려진 함정 — UI (실제로 터졌던 것들)

> 코드 편집·배포 쪽 함정(`<script>` 3개, minify 치환, 토큰 이름 가려짐, 문자열 인코딩,
> 배포 확인, GitHub Pages ETag, base64 RangeError)은 **CLAUDE.md "코드베이스 함정"**에
> 있습니다.
...
### 7-5. ⭐ 특이사항을 추가하면 "날짜칸 아이콘 목록"에도 넣어야 한다

`특이사항`(`log.special`)은 **세 곳**에서 따로 읽습니다. 새 항목을 추가할 때 한 곳만
고치면 나머지가 조용히 빠집니다 — `dining`(회식·외식)이 실제로 두 곳에서 빠져 있었습니다
(2026-09-09 제보 "외식 아이콘 날짜밑에 안나옴").

| 위치 | 코드 모양 | 빠지면 생기는 일 |
|---|---|---|
| 기록 화면 토글 | `[["drinking",...],["dining",...]]` | 체크 자체를 못 함 |
| 하루 요약 칩 | `Object.entries({drinking:[...],dining:[...]})` | 칩이 안 보임 |
| **주간 날짜칸 아이콘** | `f1=[...L1.drinking?"drinking":null]` | **날짜 밑 아이콘이 안 나옴** |
| 전체기록 검색 | `{drinking:"음주",...}[k]` | 그 말로 검색해도 안 찾힘 |

**날짜칸은 폭이 48.6px(390px 기준)뿐입니다.** 아이콘을 `size:11`+`gap:2`로 고정해두면
4개부터 칸을 넘습니다(실측: 5개 7.2px 삐져나감 — 이미 있던 문제였습니다). 지금은 개수에
따라 크기·간격을 줄여 6개까지 넘침 0입니다. **아이콘을 또 추가하면 이 계단값을 다시 잴 것.**

### 7-6. ⭐ 슬롯 뱃지를 빈 문자열로 폴백하지 말 것

`SB[x.slot]||["",c.midGray]` — `slot`이 없는 옛 복약 묶음이 **라벨 없는 빈 회색 뱃지**로
보입니다("약이 아침약 저녁약 안 나뉘어있다"는 제보의 한 갈래). 하루 화면은 이미
`slot!=="am"` → 저녁으로 묶고 있으므로, 뱃지도 **같은 규칙으로** 폴백해야 화면끼리 어긋나지
않습니다: `SB[x.slot]||SB[x.type==="prn"?"prn":"pm"]`.

복약 관리 모달은 등록 순서 그대로 `Rm.map`이라 아침/저녁이 섞여 보였습니다 — 다른 화면과
같은 **아침 → 저녁 → 필요시** 순으로 정렬합니다. 목록을 새로 만들 땐 이 순서를 따를 것.

### 7-7. ⭐ "펼침 규칙"은 하나뿐이어야 한다 — 등록 여부로 열지 말고 오늘 기록으로 열 것

`복약` 카드만 다른 세 카드(생리·특이사항·메모)와 다른 기준으로 펼쳐져 있었습니다
("복용 스크롤 너무 긴데" 제보, 2026-09-10):

| 카드 | `defaultOpen` 기준 |
|---|---|
| 생리 | `!!t.period` — **오늘 기록에** 이미 값이 있으면 펼침 |
| 특이사항 | `!!(t.special && ...)` — **오늘 기록에** 이미 값이 있으면 펼침 |
| 메모 | `!!(t.notes && ...)` — **오늘 기록에** 이미 값이 있으면 펼침 |
| 복약(고침 전) | `!!(mr\|\|[]).length` — **레지스트리**가 비어있지 않으면 펼침 |

레지스트리는 약을 한 번이라도 등록하면 그 뒤로 영원히 비지 않으므로, 이 카드는
**사실상 매일 강제로 펼쳐진 채** 시작했습니다. 매일 자동으로 "복용"으로 잡히는 약이
대부분이라 정작 손댈 일이 없는 날에도 8~15개 행이 스크롤을 다 차지했습니다.

`오늘 기록에 이미 뭔가 있는가`로 통일: `!!((t.medOv&&Object.keys(t.medOv).length)
||(t.meds&&t.meds.length))` — 오늘 용량을 바꾸거나(`medOv`) 필요시 약을 하나라도
체크한 날에만 펼친 채로 시작하고, 평범한 날은 접힌 채로 시작해 언제든 탭해서 열 수
있습니다. **새 카드에 `defaultOpen`을 넣을 때 "레지스트리/설정이 비어있는가"로 재지
말 것 — 항상 "오늘 기록에 이미 값이 있는가"로 잴 것.**

### 7-8. 복약 행 — "n종 · 눌러서 상세" 캡션은 이 폼의 다른 접기 헤더들과도 안 맞았다

같은 화면의 `신체 측정`·`특이사항`·`메모` 헤더는 전부 **화살표(▾)만** 있고 설명 텍스트가
없는데, 복약 행만 매번 "N종 · 눌러서 상세"라는 캡션 줄을 따로 그려 다품목 약마다 한 줄이
더 붙었습니다. 이름 줄 안에 작은 "N종" 배지 + 화살표만 남기고 캡션 줄을 없애 다른 헤더들과
같은 밀도로 맞췄습니다(실측: 행 높이 52~57.6px → 전부 48px로 통일). `hd`(상세 보유 여부)
판정 로직은 그대로 — 화살표를 눌러 펼치면 상세(`det(x)`)는 그대로 나옵니다.

### 7-9. ⭐ 홈 화면에 설치한 상태에서는 `<a download>`가 조용히 실패한다

같은 코드가 **Safari 탭에서는 파일을 저장하고, 홈 화면 앱(standalone)에서는 아무 일도
일어나지 않습니다.** WKWebView가 `download` 속성이 붙은 앵커의 `blob:` URL을 처리하지
않기 때문입니다([WebKit #216918](https://bugs.webkit.org/show_bug.cgi?id=216918), 미해결).
에러도, 콘솔 로그도 없습니다 — 버튼이 먹통인 것처럼 보일 뿐입니다.

2026-09-10에 "직접 다운로드로 저장" 링크를 보조로 넣었을 때 이 차이를 캡션으로만
경고했는데(*"홈 화면에 설치해 쓰는 중이면 안 될 수 있어요"*), 실제로는 **민영님이 쓰는
경로가 바로 그 홈 화면 앱**이라 그 링크는 처음부터 동작할 수 없었습니다. 추측을 사용자에게
떠넘기지 말고 **앱이 자기 실행 모드를 판정해 경로를 갈라야 합니다.**

```js
sa = window.navigator.standalone === true ||
     !!(window.matchMedia && window.matchMedia("(display-mode: standalone)").matches);
if (!sa) { dl(); return }          // Safari 탭 → 바로 다운로드(설정된 폴더로)
// 홈 화면 앱 → 공유시트(navigator.share)만이 유일한 출구
```

같은 이유로 **공유시트를 취소했을 때 `dl()`로 폴백하면 안 됩니다.** standalone에서는 그
폴백이 조용히 실패하는데 "백업 완료"로 기록돼 *마지막 백업 날짜만 갱신되고 파일은 없는*
상태가 됩니다. 취소는 취소로 두세요.

**시도했다가 되돌린 것 — 클립보드 + `shortcuts://` 직행(2026-09-18).** 공유시트조차 건너뛰려고
암호화본을 클립보드에 싣고 `shortcuts://run-shortcut?name=…&input=clipboard`로 넘겨봤습니다.
기술적으로는 동작했지만 **실사용에서 더 나빴습니다** — iOS가 붙여넣기 허용을 묻고, 단축어 앱이
전면으로 떴다가 돌아오고, 텍스트 입력이라 파일 이름을 다시 확인받는 화면까지 붙어서 탭이
오히려 늘었습니다. 되돌렸습니다. 다시 시도하지 말 것 — **웹앱에서 단축어를 조용히 실행할
방법은 없습니다.** 공유시트(`navigator.share`)가 홈 화면 앱의 유일하고 가장 짧은 출구입니다.
(데이터를 URL `&text=`에 싣는 변형도 안 됩니다. 백업이 160KB대라 조용히 실패합니다.)

### 7-10. 매번 묻는 암호 프롬프트는 "한 번에 끝나는 버튼"을 막는다

백업은 자주 눌러야 의미가 있는데 누를 때마다 `prompt()`가 뜨면 타수가 늘고 오타로
복호화가 실패합니다. 지금은 첫 백업에서 한 번만 묻고 `confirm`으로 동의를 받아
`localStorage.ml_bk_pw`에 저장합니다. 모달 하단에 **"암호 기억됨 · 지우기"** 를 항상
노출해 되돌릴 수 있게 둘 것 — 저장해놓고 지울 방법이 없으면 안 됩니다.

> 이 저장은 **파일 암호화를 약화시키지 않습니다.** 같은 `localStorage`에 이미 기록
> 원본이 평문으로 들어 있어, 폰이 잠금해제된 상태로 넘어가면 어차피 다 보입니다.
> 이 암호가 지키는 건 **iCloud로 나간 백업 파일**입니다.
...
1. ⚠️ **Border-radius / 버튼 높이 전체 스케일 통일** — radius 15종, 높이 40종이 60곳 넘게
   ...
   하나씩 확인하며 진행할 것.
2. ✅ **필요시 행의 복용 버튼 — 2026-09-10 §6-4 규격으로 통일 완료.** `height:30,
   padding:"0 9px",borderRadius:8,fontSize:11`로 매일 복용 행과 정확히 같은 모양입니다.
   색만 상태 의미에 맞게 갈랐습니다: **활성("✓ 복용")은 매일 행의 "복용" 상태와 같은
   라벤더 틴트**(`border:1.5px solid ${c.lavender}66`, `background:${c.lavender}15`,
   `color:c.navy`) — 둘 다 "그 약을 먹었다"는 같은 의미라 같은 색을 씁니다. **비활성
   ("미복용")은 매일 행의 "거름"(pink 경고색)을 그대로 빌리지 않았습니다** — 매일 약의
   "거름"은 예정된 복용을 건너뛴 이탈이지만, 필요시 약의 "미복용"은 대부분의 날에 해당하는
   평범한 기본 상태라 경고색을 칠하면 거짓 신호가 됩니다. 대신 앱 전역의 "비활성 칩" 관례
   (`TK0.line` 보더 · `#fff` 배경 · `#9ca3af` 글자)를 그대로 씁니다.
...
- **`D2`** (증상) — `nausea, vomiting, bodyache, heat, diarrhea_s, constipation,
  ...
  **★2026-09-26 — `anxiety`/`stress`는 Material 원본을 그대로 쓰되 weight·배율만 맞춘다★**
  9/21엔 `sentiment_worried`/`sentiment_stressed`의 **filled** 변형을 썼는데, 다른 D2
  얼굴들이 전부 hollow-line이라 혼자 꽉 찬 원으로 튀었다. 지금은 같은 두 아이콘의
  **`rounded/` 폴더 · 비-fill(라인) 변형 · weight 300**을 쓰고 **`(12,12) 중심으로 1.1606배**
  확대해 넣는다. 근거는 측정값이다:
  | | 폭 | 좌우 선 두께 |
  |---|---|---|
  | 기존 얼굴들(nausea·orthostatic) | 21.95 | 1.45~1.50 |
  | w400 라인 @1.10배 | 21.95 | **1.65**(너무 굵음) |
  | w300 라인 @1.1606배 | **22.05** | **1.35** ✔ |
  `rounded/` 폴더를 쓰는 이유는 **선 끝이 둥근 변형이기 때문**이다(사용자 요구 "라인 끝은
  rounded"). `outlined/`는 끝이 각지게 잘린다.
  **★표정을 직접 그려 넣지 말 것★** — 한 번 Material의 원을 버리고 눈·눈썹·입을 손으로
  그려 끼웠다가 "안으로 파였다"고 반려됐다. 원본 글리프를 통째로 쓰고 **배율만** 조정할 것.
  - `edema`(부종, 2026-09-26 신설) — **다른 D2 얼굴들과 달리 유일하게 solid filled**다
    (사용자 지시: "풍선, 까만색, 삼각형 매듭, 라인으로 광택"). 타원 몸통 + 아래로 넓어지는
    삼각 매듭(꼭짓점이 풍선 쪽=위, 넓은 변이 아래) + 왼쪽 위 광택.
    **★몸통과 매듭을 별개 서브패스로 겹쳐 놓지 말 것★** — D2는 항상 `evenodd`라 겹친
    부분이 XOR로 **뚫린다**(실제로 흰 홈이 생겨 반려됨). `pipeline` 없이 해결하려면
    **선-타원 교점을 직접 계산해 합집합 외곽선 하나로** 그려야 한다(삼각형 두 빗변이 타원과
    만나는 점을 구하고, 그 두 점 사이는 타원 호를 '위로 돌아' 잇는 폴리곤 — 기존 손 그림
    아이콘들도 전부 다각형이라 스타일이 어긋나지 않는다).
    광택은 **안쪽 타원 호를 따라 샘플링한 둥근 끝 선**이라 풍선 곡률과 나란하다(직선으로
    그리면 따로 논다).
