...
```js
접기:  y > 170   (그리고 스크롤 가능 높이 k >= 300)
펼치기: y < 100
```

70px 히스테리시스 구간(100~170)이 있어 경계에서 왕복하지 않습니다.
...
그래서 펼치기 임계값을 `y<24`가 아니라 **`y<100`** 으로 잡습니다. 위로 올라오는 도중
100px 남은 지점에서 시작하므로 남은 스크롤에 움직임이 얹힙니다. 방향 기반(누적 상승량)으로
가면 위 항목의 민감도 문제가 생기니 **위치 기반으로 구간만 넓히는 선에서** 해결하세요.
